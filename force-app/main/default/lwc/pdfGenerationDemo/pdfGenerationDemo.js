...
import generatePDF from '@salesforce/apex/pdfControler.generatePDF'
export default class PdfGenerationDemo extends LightningElement {
    @api recordId
    imageUrl = "https://images.fastcompany.net/image/upload/w_596,c_limit,q_auto:best,f_auto/fc/3034007-inline-i-applelogo.jpg"
    invoiceData={
        invoiceNo:'123',
        ...
        address2:'sunnyVillage, CA 132124'
    }
    clientData={
        client: 'Acme Corp',
        username:'John Doe',
...
pdfHandler(){
    ...
    generatePDF({recordId:this.recordId, htmlData:content.outerHTML}).then(result=>{
        ...
        window.open(`https://computing-ruby-5530-dev-ed--c.documentforce.com/servlet/servlet.FileDownload?file=${result.Id}`)
    }).catch(error=>{
        console.error(error)
    })
}
